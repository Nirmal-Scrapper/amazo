...
<body bgcolor="orange">
	...
	<form action="shopping.do" method="post">
		...
		//	System.out.print(
		//	"<input type=checkbox name='" + items[i + 6][0] + "' value='" + items[i + 6][1] + "'>" + items[i + 6][1]);
			out.println("<img src='" + items[i + 6][3] + "' alt='" + items[i][0] + "' style='width:50px;height:50px'/>");
			out.println(
			"<input type=checkbox name='" + items[i + 6][0] + "' value='" + items[i + 6][1] + "'>" + items[i + 6][1]);
			int limit = Integer.parseInt(items[i + 6][2]);
			if (limit > 10) {
				limit = 10;
			}
			out.println("  <input type='range' name='" + items[i+6][0] + "quan' min='0' max='" + limit + "' value='0' class='slider' id='" + items[i + 6][0]
			+ "'> <br>");
			out.println(" <p>quantity: <span id='demo" + i + "'></span></p>");
			out.println("<script>");
			out.println("var slider" + i + " = document.getElementById('" + items[i + 6][0] + "');");
			...
			out.println(" document.getElementsByName('" + items[i + 6][0] + "').checked = true;");
			out.println("if(this.value >0 ){");
			out.println(" document.getElementsByName('" + items[i + 6][0] + "')[0].checked = true;");
			out.println("}");
			out.println("else{");
			out.println(" document.getElementsByName('" + items[i + 6][0] + "')[0].checked = false;");
			out.println("}");
			out.println("}");
			out.println("const checkbox" + i + " = document.getElementsByName('" + items[i + 6][0] + "')[0];");
